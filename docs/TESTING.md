...
An opt-in Docker test verifies that an SSH key generated by Voyager authenticates against a real OpenSSH-backed SFTP container, uploads and downloads a probe file, and creates and extracts a ZIP through the remote provider:
...
The test uses the `atmoz/sftp:alpine` image, publishes a random loopback port, and always force-removes its uniquely named container during teardown.
