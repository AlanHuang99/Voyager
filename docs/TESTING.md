...
## September browser feedback regression coverage

`BrowserFeedbackRegressionTest` checks stationary long presses across toolbar layout changes, intentional range selection, list/grid scroll restoration after parent navigation and refresh, independent session positions, rejection of late scroll updates, compact and bottom search, and selected-folder bookmark/shortcut/duplicate actions. `BrowserLocationSheetTest` checks recent bookmarks, Show all, system Back, in-place sessions, and Home navigation. `PreferenceWriteReloadTest` verifies that unrelated preference writes do not reload a directory. `BookmarkMigrationTest` opens a version-2 database with saved bookmarks and connections through Room version 3 and verifies their preservation and subsequent recent-use updates. `SplashThemeTest` checks the explicit Voyager icon and day/night background resources. Huawei-specific launcher rendering still requires confirmation on an affected EMUI device.
...
Run `RootFileProviderAndroidTest` and `RootAccessUiTest` for Android toybox compatibility, child-process timeout/cancellation and blocked-stream cleanup, explicit root confirmation, visible denial, ordinary local browsing, staged editor saves, discard, and permanent-delete confirmation. `RootEditorOperationGateTest` blocks a real move after its destination captures the source bytes, then verifies that Save cannot overwrite that source; it also verifies that service-start failure keeps the draft and supports retry. The command compatibility and editor fixtures deliberately use an unprivileged shell in the debug app cache; they do not claim successful superuser access. Successful privileged Android operation must also be checked on a rooted disposable device, including SELinux labels and owner/mode preservation. On the September 8 K60, `su -c id` returned `Permission denied`; only denial and unprivileged Android command/UI behavior can be verified there.
