...
| WebDAV | Slash-delimited remote path | Uses Sardine for metadata operations. Coordinator transfers stream through an OkHttp request body so progress measures the HTTP upload; generic output streams use a temporary-file request body when a caller requires the `OutputStream` interface. |
